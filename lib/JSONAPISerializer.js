...
module.exports = class JSONAPISerializer {
  ...
  serialize(type, data, schema, extraData) {
    ...
    const serializedIncluded = this.serializeIncluded(included);
    ...
    return {
      ...
      data: serializedData,
      included: extraData.include && !serializedIncluded ? [] : serializedIncluded,
    };
  }
    ...
    return streamToArray(dataStream)
      ...
      .then((result) => {
        ...
        return {
          ...
          included: extraData.include && !serializedIncludes ? [] : serializedIncludes,
        };
      });
  }
   ...
   */
  serializeData(type, data, options, included, extraData, path) {
    // Empty data
    if (_.isEmpty(data)) {
      // Return [] or null
    ...
    if (Array.isArray(data)) {
      return data.map(d => this.serializeData(type, d, options, included, extraData, path));
    }
    ...
    return {
      ...
      relationships: this.serializeRelationships(data, options, included, extraData, path),
   ...
   */
  serializeRelationships(data, options, included, extraData, path) {
    const serializedRelationships = {};

    Object.keys(options.relationships).forEach((relationship) => {
      const rPath = path ? `${path}.${relationship}` : relationship;
      ...
      const serializeRelationship = {
        ...
        data: this.serializeRelationship(rOptions.type, data[relationshipKey], this.schemas[rOptions.type][schema], included, extraData, rPath),
      };
   ...
   */
  serializeRelationship(rType, rData, rOptions, included, extraData, path) {
    ...
    if (Array.isArray(rData)) {
      return rData.map(d => this.serializeRelationship(rType, d, rOptions, included, extraData, path));
    }
    ...
    if (!_.isObjectLike(rData)) {
      ...
      if (extraData && extraData.include) {
        const found = extraData.include.find(includePath => includePath.startsWith(path));
        if (found) {
          included.push(this.serializeData(rType, rData, rOptions, included, extraData, path));
        }
      } else {
        included.push(this.serializeData(rType, rData, rOptions, included, extraData, path));
      }
