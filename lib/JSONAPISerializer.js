...
module.exports = class JSONAPISerializer {
  ...
  serializeRelationships(data, options, included, extraData) {
    ...
    Object.keys(options.relationships).forEach((relationship) => {
      ...
      let serializeRelationship = {
        ...
        data: this.serializeRelationship(rOptions.type, rOptions.schema, _.get(data, relationshipKey), included, extraData),
      };
   ...
   */
  serializeRelationship(rType, rSchema, rData, included, extraData) {
    ...
    if (Array.isArray(rData)) {
      return rData.map(d => this.serializeRelationship(rType, schema, d, included, extraData));
    }

    // Resolve relationship type
    const type = (typeof rType === 'function') ? rType(rData) : rType;
