    ...
    if (data.relationships) {
      Object.keys(data.relationships).forEach((relationship) => {
        ...
        if (_.isArray(value.data)) {
          _.set(deserializedData, relationshipKey, value.data.map(d => (included
            ? this.deserializeIncluded(d.type, d.id, resourceOpts.relationships[relationship], included)
            : d.id)));
        } else if (value.data !== null) {
          _.set(deserializedData, relationshipKey, included
            ? this.deserializeIncluded(value.data.type, value.data.id, resourceOpts.relationships[relationship], included)
            : value.data.id);
        }
      });
    }
